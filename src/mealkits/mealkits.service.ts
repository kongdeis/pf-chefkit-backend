...
export class MealkitsService {
  ...
  async create(createMealkitDto: CreateMealkitDto, member: AuthMember) {
    ...
    if (!allowedRoles.includes(role))
      throw new ForbiddenException(`밀키트 등록 권한이 없습니다.`);
    const chefId = member.id;

    // 재료 확인
    const { mealkitIngredients } = createMealkitDto;
    const ingredientIds = await mealkitIngredients.map(
    ...
    const ingredientsPrice = new Map(
      ingredients.map((i) => [i.id, i.unitPrice]),
    );
  ...
  async update(
    id: number,
    updateMealkitDto: UpdateMealkitDto,
    member: AuthMember,
  ) {
    // 권한 확인
    const { role } = await member;
    const allowedRoles = ["ADMIN", "CHEF"];
    if (!allowedRoles.includes(role))
      throw new ForbiddenException(`밀키트 수정 권한이 없습니다.`);
    console.log("---update---", updateMealkitDto);

    // 밀키트 확인
    const mealkit = await this.findOne(id);

    // 셰프 권한일 경우 밀키트를 등록한 셰프가 맞는지 확인
    if (member.role === "CHEF" && mealkit.chefId !== member.id)
      throw new ForbiddenException(`밀키트 수정 권한이 없습니다.`);
    ...
    return this.prisma.$transaction(async (tx) => {
      ...
      // 밀키트 재료 수정이 있는 경우
      const ingredientIds = updateMealkitDto.mealkitIngredients.map(
        (item) => item.ingredientId,
      );
      ...
      let totalPrice = 0;
      const ingredientItems = updateMealkitDto.mealkitIngredients.map(
        (item) => {
          const unitPrice = ingredientsPrice.get(item.ingredientId) ?? 0;
          const price = unitPrice * item.quantity;
          totalPrice += price;
  ...
  async remove(id: number, member: AuthMember) {
    ...
    const { role } = await member;
    if (role != "ADMIN")
      throw new ForbiddenException(`밀키트 삭제 권한이 없습니다.`);
    // 밀키트 가져오기
    const mealkit = await this.findOne(id);
    // 주문내역이 있으면 삭제 불가능
    if (mealkit.orderCount > 0)
      throw new BadRequestException(
        `주문내역이 있는 밀키트는 삭제할 수 없습니다`,
      );
    // 밀키트 삭제
    await this.prisma.$transaction([
      this.prisma.mealkitIngredient.deleteMany({
        where: { mealkitId: mealkit.id },
      }),
  ...
  async findOneDetail(id: number) {
    ...
    if (!mealkit)
      throw new NotFoundException(`밀키트 ${id}를 찾을 수 없습니다`);
    return mealkit;
  }
}
