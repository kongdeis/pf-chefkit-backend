...
import {
  type AuthMember,
  CurrentMember,
} from "../common/current-member.decorator";
...
export class MealkitsController {
  ...
  @ApiBearerAuth()
  @ApiOperation({ summary: "밀키트 등록은 관리자, 셰프만 가능" })
  create(
    @Body() createMealkitDto: CreateMealkitDto,
  ...
  @UseGuards(JwtAuthGuard)
  @ApiBearerAuth()
  @ApiOperation({ summary: "밀키트 수정은 관리자와 등록한 셰프만 가능" })
  update(
    @Param("id") id: string,
    @Body() updateMealkitDto: UpdateMealkitDto,
    @CurrentMember() member: AuthMember,
  ) {
